<script
    ...
    const progress = computed(() => {
        ...
        return Math.min(100, (localElapsed.value / props.nowPlaying.duration) * 100);
    });
...
<template>
    <div class="now-playing">
        ...
        <div
            ...
            class="progress-bar">
            <span class="time">{{ formatTime(localElapsed) }}</span>
            <div class="track">
                <div
                    class="fill"
                    :style="{width: progress + '%'}"></div>
            </div>
            <span class="time">{{ formatTime(nowPlaying.duration) }}</span>
        </div>
    </div>
</template>
